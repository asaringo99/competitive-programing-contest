...
struct RollingHash{
    ...
    RollingHash(set<int> S){
        ...
        int i = 0;
        for(int u : S){
            hash1[i+1] = (hash1[i] * base1 + u) % mod1 ;
            hash2[i+1] = (hash2[i] * base2 + u) % mod2 ;
    ...
    // 1 <= l , r <= n
    inline pair<int,int> get_hash(int l , int r) const {
        int res1 = (hash1[r] - (hash1[l-1] * power1[r-l+1] % mod1) + mod1) % mod1 ;
        int res2 = (hash2[r] - (hash2[l-1] * power2[r-l+1] % mod2) + mod2) % mod2 ;
        return {res1,res2} ;
    }
...
void solve(){
    ...
    F = G;
    rep(i,n) F[i].insert(i);
    rep(i,n) F[i].insert(-1);
    rep(i,n) G[i].insert(-1);
    rep(i,n){
        mpG[G[i]]++;
        mpF[F[i]]++;
    }
