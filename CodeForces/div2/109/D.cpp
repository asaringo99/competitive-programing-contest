...
void solve(){
    ...
    rep(i,q){
        ...
        if(c == '+'){
            ...
            vector<int> prime = ets.prime_factor(a);
            bool ok = false;
            int conflict = -1;
            for(int u : prime) if(!st[u].empty()){
                auto it = st[u].begin();
                conflict = *it;
                ok = true;
            }
            if(ok) cout << "Conflict with " << conflict << endl;
            else{
                used.insert(a);
                for(int u : prime) st[u].insert(a);
                cout << "Success" << endl;
            }
        }
        else{
