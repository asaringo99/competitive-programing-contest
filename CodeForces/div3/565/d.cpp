...
void solve(){
    int n;
    cin >> n;
    Eratosthenes ets(3030303);
    vector<int> B(2*n), C(2*n), rem, A;
    set<int> used;
    rep(i,2*n) used.insert(i);
    rep(i,2*n) cin >> B[i], C[i] = B[i];
    sort(C.begin(),C.end());
    for(int i = 2 * n - 1 ; i >= 0 ; i--){
        if(used.count(i) == 0) continue;
        int val = C[i];
        if(ets.is_prime(val)) continue;
        used.erase(i);
        vector<int> pf = ets.prime_factor(val);
        sort(pf.begin(),pf.end());
        A.push_back(val);
        val /= pf[0];
        auto it = lower_bound(C.begin(),C.end(),val);
        int id = it - C.begin();
        auto ite = used.lower_bound(id);
        used.erase(ite);
    }
    vector<int> prm = ets.get_prime();
    rep(i,2*n){
        if(used.count(i) == 0) continue;
        int val = C[i];
        auto it = lower_bound(C.begin(),C.end(),prm[val-1]);
        int id = it - C.begin();
        auto ite = used.lower_bound(id);
        used.erase(ite);
        A.push_back(val);
