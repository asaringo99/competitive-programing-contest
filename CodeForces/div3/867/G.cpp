...
int main(){
    ...
    rep(i,t){
        ...
        ll res = 0;
        rep(i,n){
            res += (mp[A[i]] - 1) * (mp[A[i]] - 2);
            if(A[i] >= 1000000){
                rrep(x,2,1001){
                    if(A[i] * x > 1000000000) break;
                    ll s = 0, t = 0;
                    if(A[i] % x == 0 && mp.count(A[i]/x)) s = mp[A[i]/x];
                    if(mp.count(A[i]*x)) t = mp[A[i]*x];
                    res += s * t;
                }
            }
            else{
                for(ll x = 1; x * x <= A[i]; x++){
                    ...
                    if(x * x != A[i]){
                        ll s = 0, t = 0;
                        ll y = A[i] / x;
                        if(mp.count(A[i]/y)) s = mp[A[i]/y];
                        if(mp.count(A[i]*y)) t = mp[A[i]*y];
                        res += s * t;
                    }
                    // cout << A[i] << " " << x << " " << s << " " << t << endl;
                }
