...
namespace monoid{
    ...
    struct M{ ll a; };
    M id() { return M{0}; }
    S mapping(M x , S y) { return S{ x.a * y.size + y.val, y.size }; }
...
int main(){
    ...
    cin >> n >> q;
    vector<S> V(n,{0,1});
    LazySegmentTree<S, op, e, M, mapping, composition, id> segtree(V);
    ...
    rep(i,q){
        ...
        if(t == 1){
            ...
            cout << HLD.query(u,v,e(),calc,merge,false).val << endl;
        }
    }
}