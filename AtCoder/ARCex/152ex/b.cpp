...
void solve(){
    ...
    rep(i,n) B.push_back({-2*L+A[i], i});
    rep(i,n) B.push_back({-A[n-1-i], n-1-i});
    rep(i,n) B.push_back({A[i], i});
    rep(i,n) B.push_back({L+L-A[n-1-i], n-1-i});
    rep(i,n) B.push_back({L+L+A[i], i});
    ll res = 1e18;
    rep(i,n){
        ll a = (A[i] + L) % L;
        ll b = L - A[i];
        ...
        a = A[i] + L;
        b = A[i] - L;
        auto at = upper_bound(all(B),pair<ll,ll>(a,-1)); at--;
        auto bt = upper_bound(all(B),pair<ll,ll>(b,-1));
        int aid = at->second;
        int bid = bt->second;
        ll dista = at->first - A[i];
        ll distb = A[i] - bt->first;
        ll xx = bt->first; bt--;
        ll yy = bt->first;
        ll dist = abs(xx - yy);
        ll value = 2 * L + dist - abs(dista - distb);
        // cout << i << " " << bid << " " << aid << " " << value << " " << dist << endl;
        chmin(res, value);
    }
    pt(res)
}
