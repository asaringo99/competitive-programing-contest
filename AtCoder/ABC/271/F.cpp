...
vector<int> X[22];
hash_map<int,ll> Y[22];

void bfs(){
    queue<TP> que;
    que.push({0,0,0,0});
    while(!que.empty()){
        auto[x,y,val,cnt] = que.front(); que.pop();
        val ^= A[x][y];
        if(cnt + 1 == n) {
            X[x].push_back(val);
            continue;
        }
        if(x + 1 < n) que.push({x+1,y,val,cnt+1});
        if(y + 1 < n) que.push({x,y+1,val,cnt+1});
    }
}
...
void bfs2(){
    ...
    while(!que.empty()){
        ...
        if(x - 1 >= 0) que.push({x-1,y,val,cnt+1});
        if(y - 1 >= 0) que.push({x,y-1,val,cnt+1});
    }
}
