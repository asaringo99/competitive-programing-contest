...
int main(){
    ...
    modint res = 0;
    int sum = 0;
    rep(i,3030) rep(j,3030) powlist[i][j] = powmod(i,j);
    rrep(x,1,m+1){
        int pre = sum; sum += st.count(x);
        int now = sum;
        modint a = 0, b = 0;
        rep(i,p+1){
            if(now + i >= k) a += combination(p,i) * powlist[x][i] * powlist[m-x][p-i];
            if(pre + i >= k) b += combination(p,i) * powlist[x-1][i] * powlist[m-x+1][p-i];
        }
        res += (a - b) * x;
    }
    res /= powmod(m,p);
