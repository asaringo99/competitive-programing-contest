...
int main(){
    ...
    rep(i,H){
        rep(j,W){
            ...
            sum += combination(H * j, k);
            sum += combination(W * i, k);
            sum += combination(H * (W - j - 1), k);
            sum += combination(W * (H - i - 1), k);

            sum -= combination(i * j, k);
            sum -= combination(i * (W - j - 1), k);
            sum -= combination(j * (H - i - 1), k);
            sum -= combination((W - j - 1) * (H - i - 1), k);

            res += combination(H * W, k) - sum;
        }
    }
    res /= combination(H * W, k);
    cout << res << endl;
}