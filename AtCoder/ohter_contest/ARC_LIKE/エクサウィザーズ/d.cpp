...
int n, x;
modint dp[101010][202]; // iという値に対してj個残りがある時の場合の数
ll S[202];
ll minval;

modint rec(int x, int r){
    if(dp[x][r] != -1) return dp[x][r];
    if(x < minval) return dp[x][r] = modint(x) * permutation(r,r);
    modint res = 0;
    rep(i,n){
        if(S[i] > x) break;
        int md = x % S[i];
        int nm = upper_bound(S,S+n,md) - S;
        res += rec(md, nm) * permutation(r - 1, r - 1 - nm);
    }
    return dp[x][r] = res;
}
...
int main(){
    ...
    rep(i,n) if(S[i] > x) c++;
    if(c == n){
        cout << permutation(n,n) * x << endl;
        return 0;
    }
    minval = S[0];
    rep(i,101010) rep(j,202) dp[i][j] = -1;
    modint res = rec(x,n-c) * permutation(n,c);
    cout << res << endl;
}