...
int main(){
    ...
    rrep(x,1,n+1) rrep(y,1,m+1){
        // if(max(x,y) > b) continue ;
        // if(x * y < b) continue ;
        ll sum = 0 ;
        rrep(i,1,x+1) rrep(j,1,y+1){
            if(((x+y)-(i+j))%2==0) (sum += combination(i*j,b) * combination(x,i) % mod * combination(y,j) % mod) %= mod ;
            if(((x+y)-(i+j))%2==1) (sum -= combination(i*j,b) * combination(x,i) % mod * combination(y,j) % mod) %= mod ;
        }
        (sum += mod) %= mod ;
        (sum *= combination(n,x)) %= mod ;
        (sum *= combination(m,y)) %= mod ;
