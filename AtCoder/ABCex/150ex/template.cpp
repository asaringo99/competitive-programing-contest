...
int main(){
    ...
    cin >> n ;
    int m = n - 1;
    vector<int> A(n), B(n), X(2*n-1), Y(n-1), Z(2*n-1+m);
    rep(i,n) cin >> A[i];
    rep(i,n) cin >> B[i];
    rep(i,2*n-1) X[i] = (A[i%n] ^ A[(i+1)%n]);
    rep(i,m) Y[i] = (B[i] ^ B[i+1]);
    rep(i,2*n-1) Z[i] = X[i];
    rep(i,m) Z[i+2*n-1] = Y[i];
    SuffixArray sa(Z);
    sa.build();
    vector<pair<int,int>> res;
    rep(i,n){
        if(sa.get_lcp(2*n-1,i) == m){
            res.push_back({i,(A[i]^B[0])});
        }
    }
