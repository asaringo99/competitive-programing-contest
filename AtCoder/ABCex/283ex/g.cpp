...
int main(){
    ...
    A.resize(n);
    rep(i,n) cin >> A[i];
    vector<vector<bool>> B(n,vector<bool>(60,0));
    rep(i,n){
        rep(j,60) if(A[i] >> j & 1) B[i][j] = true;
    }
    Matrix M(B);
    GaussJordan gj;
    vector<vector<bool>> vec = gj.create_xorbase(M,true);
    int rank = gj.get_rank();

    vector<ll> arr;
    rep(i,rank){
        ll val = 0;
        rep(j,60) if(vec[i][j]) val |= ((ll)1 << j);
        arr.push_back(val);
    }
    sort(all(vec));
    // for(ll u : arr) cout << u << endl;
    L--;
    rrep(i,L,R){
        ll val = 0;
        rep(j,60) if(i >> j & 1) val ^= arr[j];
        cout << val << " ";
    }
    cout << endl;
}