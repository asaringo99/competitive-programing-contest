...
ll bfs(){
    ...
    while(!que.empty()){
        ...
        cout << x << " " << y << " " << t << endl;
        if(t == 0){
            auto it = upper_bound(Y[y].begin(),Y[y].end(),y);
            if(it != Y[y].end()){
                int nx = *it;
                ll val = abs(nx - x);
                if(!d[0].count(P(nx,y))){
                    d[0][P(nx,y)] = d[0][P(x,y)] + val;
                    que.push({nx,y,0});
                }
                else{
                    if(d[0][P(nx,y)] > d[0][P(x,y)] + val){
                        d[0][P(nx,y)] = d[0][P(x,y)] + val;
                        que.push({nx,y,0});
                    }
                }
            ...
            if(nx == x){
                if(!d[1].count(P(x,y))) {
                    d[1][P(x,y)] = d[0][P(x,y)] + 1;
                    que.push({x,y,1});
                }
                else{
                    if(d[1][P(x,y)] > d[0][P(x,y)] + 1){
                        d[1][P(x,y)] = d[0][P(x,y)] + 1;
                        que.push({x,y,1});
                    }
                }
            }
            else if(it != Y[y].begin()) it--;
            nx = *it;
            ll val = abs(nx - x);
            if(!d[0].count(P(nx,y))){
                d[0][P(nx,y)] = d[0][P(x,y)] + val;
            ...
            auto it = upper_bound(X[x].begin(),X[x].end(),x);
            cout << "pp: " << *it << endl;
            if(it != X[x].end()){
                int ny = *it;
                ll val = abs(ny - y);
                if(!d[1].count(P(x,ny))){
                    d[1][P(x,ny)] = d[1][P(x,y)] + val;
                    que.push({x,ny,1});
                }
                else{
                    if(d[1][P(x,ny)] > d[1][P(x,y)] + val){
                        d[1][P(x,ny)] = d[1][P(x,y)] + val;
                        que.push({x,ny,1});
                    }
                }
            ...
            if(ny == x){
                if(!d[0].count(P(x,y))) {
                    d[0][P(x,y)] = d[1][P(x,y)] + 1;
                    que.push({x,y,0});
                }
                else{
                    if(d[0][P(x,y)] > d[1][P(x,y)] + 1){
                        d[0][P(x,y)] = d[1][P(x,y)] + 1;
                        que.push({x,y,0});
                    }
                }
            }
            else if(it != X[x].begin()) it--;
            ny = *it;
            ll val = abs(ny - y);
            if(!d[1].count(P(x,ny))){
                d[1][P(x,ny)] = d[1][P(x,y)] + val;
    ...
    return min(d[0][P(n-1,m-1)],d[1][P(n-1,m-1)]) == 0 ? -1 : min(d[0][P(n-1,m-1)],d[1][P(n-1,m-1)]);
}
