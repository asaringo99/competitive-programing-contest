...
int main(){
    ...
    cin >> n >> m;
    rep(i,n) cin >> A[i];
    Eratosthenes fpf(202020);
    rep(i,n){
        int a = A[i];
        vector<int> vec = fpf.prime_factor(a);
        int k = vec.size();
        rep(S,1<<k){
            if(S == 0) continue;
            int c = 1;
            rep(j,k) if(S >> j & 1) c *= vec[j];
            T[c]++;
        }
    }
    cout << n << endl;
    rrep(i,2,m+1){
        int x = i;
        vector<int> vec = fpf.prime_factor(x);
        int k = vec.size();
        int res = 0;
        rep(S,1<<k){
            if(S == 0) continue;
            int c = 1;
            rep(j,k) if(S >> j & 1) c *= vec[j];
            if(bit_count(S) % 2 == 1) res += T[c];
            if(bit_count(S) % 2 == 0) res -= T[c];
        }
        cout << n - res << endl;
    }
}